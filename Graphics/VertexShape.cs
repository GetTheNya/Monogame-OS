...
public struct VertexShape : IVertexType {
    public VertexShape(Vector3 position, Vector2 textureCoordinate, float shape, Color c1, Color c2, float thickness, float sdfSize, float pixelSize, float height = 1.0f, float aaSize = 2f, float rounded = 0f, float rotation = 0f, float blurOpacity = 1f) {
        if (thickness <= 0f) {
            c2 = c1;
            thickness = 0f;
        }
        ...
        Meta2 = new Vector4(pixelSize, aaSize, rounded, rotation);
        Meta3 = new Vector4(blurOpacity, 0, 0, 0);
    }

    public VertexShape(Vector3 position, Vector2 textureCoordinate, Color c1) {
        Position = position;
        TextureCoordinate = textureCoordinate;
        Color1 = c1;
        Color2 = c1;
    ...
    static VertexShape() {
        var elements = new VertexElement[] {
            ...
            new VertexElement(sizeof(float) * 13 + sizeof(int) * 2, VertexElementFormat.Vector4, VertexElementUsage.TextureCoordinate, 3)
        };
