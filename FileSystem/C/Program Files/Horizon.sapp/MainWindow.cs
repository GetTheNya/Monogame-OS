...
public class BrowserWindow : Window {
    ...
    private Dictionary<int, DownloadWindow> _downloads = new();
    private void AddNewTab(string url) {
        ...
        browser.OnDownloadStarted += (item, callback) => {
            Task.Factory.StartNew(() => {
                var confirmWin = new DownloadConfirmationWindow(item, callback, (selectedPath) => {
                    // This is called when user clicks 'Save' in confirmation
                    var dlWin = new DownloadWindow(item);
                    Shell.UI.OpenWindow(dlWin, owner: OwnerProcess);
                    _downloads[item.Id] = dlWin;
                });
                GetOwnerProcess().Application.OpenModal(confirmWin);
            }, System.Threading.CancellationToken.None, TaskCreationOptions.None, uiScheduler);
        };

        browser.OnDownloadUpdated += (item, callback) => {
            if (_downloads.TryGetValue(item.Id, out var dlWin)) {
                // Ensure UI updates happen on the main thread using captured scheduler
                Task.Factory.StartNew(() => {
                    if (_downloads.ContainsKey(item.Id)) { 
                        dlWin.UpdateProgress(item);
        ...
        browser.OnShowDialog += (type, message, def, callback, isUnload) => {
            if (type == BrowserControl.DialogType.Alert) {
                Shell.UI.OpenWindow(new TheGame.Core.UI.MessageBox("Horizon", message, MessageBoxButtons.OK, (res) => callback.Continue(true, null)));
            } else if (type == BrowserControl.DialogType.Confirm) {
                Shell.UI.OpenWindow(new TheGame.Core.UI.MessageBox("Horizon", message, MessageBoxButtons.YesNo, (res) => callback.Continue(res, null)));
            } else if (type == BrowserControl.DialogType.Prompt) {
                Shell.UI.OpenWindow(new InputDialog("Horizon", message, def, (res) => callback.Continue(res != null, res)));
            }
        };

        browser.OnFileDialog += (mode, title, defaultPath, filters, callback) => {
            // Map CefFileDialogMode to FilePickerMode
            var pickerMode = FilePickerMode.Open;
            if (mode == CefFileDialogMode.OpenMultiple) pickerMode = FilePickerMode.Open; 
            else if (mode == CefFileDialogMode.Save) pickerMode = FilePickerMode.Save;
            
            Shell.UI.OpenWindow(new FilePickerWindow(title, defaultPath, "", pickerMode, (path) => {
                if (!string.IsNullOrEmpty(path)) {
                    // Convert virtual selection to host path for CefSharp
                    string hostPath = VirtualFileSystem.Instance.ToHostPath(path);
                    callback.Continue(new List<string> { hostPath });
                } else {
                    callback.Cancel();
                }
            }), owner: OwnerProcess);
        ...
        browser.OnFocusedEvent += () => {
            // Register hotkeys when browser is focused
            Shell.Hotkeys.RegisterLocal(OwnerProcess, Keys.C, HotkeyModifiers.Ctrl, async () => {
                // Sync to OS clipboard
                var response = await browser.EvaluateScriptAsync("window.getSelection().toString()");
                if (response.Success && response.Result != null) {
                    string text = response.Result.ToString();
                    if (!string.IsNullOrEmpty(text)) Shell.Clipboard.SetText(text);
                }
                browser.ExecuteJavaScript("document.execCommand('copy');");
            }, rewriteSystemHotkey:true);

            Shell.Hotkeys.RegisterLocal(OwnerProcess, Keys.X, HotkeyModifiers.Ctrl, async () => {
                // Sync to OS clipboard
                var response = await browser.EvaluateScriptAsync("window.getSelection().toString()");
                if (response.Success && response.Result != null) {
                    string text = response.Result.ToString();
                ...
                browser.ExecuteJavaScript("document.execCommand('cut');");
            }, rewriteSystemHotkey:true);

            Shell.Hotkeys.RegisterLocal(OwnerProcess, Keys.V, HotkeyModifiers.Ctrl, () => {
                string text = Shell.Clipboard.GetText();
                if (text != null) {
                    // Escape string for JS
                    string escaped = text.Replace("\\", "\\\\").Replace("'", "\\'").Replace("\n", "\\n").Replace("\r", "");
                    browser.ExecuteJavaScript($"document.execCommand('insertText', false, '{escaped}');");
                }
            }, rewriteSystemHotkey:true);
        };

        browser.OnUnfocusedEvent += () => {
            // Unregister hotkeys when browser loses focus
            Shell.Hotkeys.UnregisterLocal(OwnerProcess, Keys.C, HotkeyModifiers.Ctrl);
            Shell.Hotkeys.UnregisterLocal(OwnerProcess, Keys.X, HotkeyModifiers.Ctrl);
            Shell.Hotkeys.UnregisterLocal(OwnerProcess, Keys.V, HotkeyModifiers.Ctrl);
        };
